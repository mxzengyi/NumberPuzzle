...
public static class BundleEditorDrawer
{
	...
	{	
		...
		GUILayout.BeginHorizontal(GetItemStyle(isSelect, HasFocuse()));
		{
            ...
            //绘制占用空间大小
            GUILayout.Label("Size:" + Profiler.GetRuntimeMemorySize(AssetDatabase.LoadAssetAtPath(assetPath , typeof(UnityEngine.Object))));
	...
	static Texture2D GetSharedIconOfDepend(string assetPath)
	{
		var bundleList = BundleManager.GetIncludeBundles(assetPath);
		...
		bundleList = BundleManager.GetRelatedBundles(assetPath);
